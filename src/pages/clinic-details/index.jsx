...
const ClinicDetail = () => {
  ...
  const clinic = demoClinics.find((item) => item.id === Number(id) && item.type === "clinic");
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-6 py-10 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow p-6">
            <h2 className="font-bold text-2xl mb-4 text-gray-800">Klinika haqida</h2>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">24/7 xizmat</span>
              <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm">Zamonaviy uskunalar</span>
              <span className="bg-purple-100 text-purple-800 px-3 py-1 rounded-full text-sm">Parking maydoni</span>
              <span className="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-sm">Wi-Fi</span>
            </div>
          </div>

          {/* SERVICES */}
          <div className="bg-white rounded-xl shadow p-6">
            <h2 className="font-semibold text-xl mb-6 text-gray-800">Xizmatlar</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {(clinic.services || ["Diagnostika", "Operatsiya", "Poliklinika", "Tez yordam", "Reabilitatsiya"]).map(
                (service, i) => (
                  <div key={i} className="bg-linear-to-br from-blue-50 to-blue-100 p-4 rounded-xl text-center hover:shadow-md transition">
          ...
          {/* ADVANTAGES */}
          <div className="bg-white rounded-xl shadow p-6">
            <h2 className="font-semibold text-xl mb-6 text-gray-800">Afzalliklar</h2>
            <ul className="space-y-4 text-gray-700">
              <li className="flex gap-3 items-center p-3 bg-green-50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Barcha shifokorlarimiz xalqaro sertifikatlarga ega</p>
                </div>
              </li>
              <li className="flex gap-3 items-center p-3 bg-red-50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Kechayu kundiz tibbiy yordam</p>
                </div>
              </li>
              <li className="flex gap-3 items-center p-3 bg-blue-50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Eng yangi diagnostika va davolash uskunalari</p>
                </div>
              </li>
            </ul>
          </div>
          ...
          <div className="bg-white rounded-xl shadow p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="font-semibold text-xl text-gray-800">Shifokorlarimiz</h2>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {clinic.doctors?.map((doc) => (
                ...
                >
                  <img 
                    src={doc.image || `https://ui-avatars.com/api/?name=${doc.name}&background=3B82F6&color=fff`} 
                    alt={doc.name}
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-blue-600 font-semibold mt-2">{doc.price} so'm</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-xl shadow p-6">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="font-semibold text-xl text-gray-800">Bemorlarning fikrlari va baholashlar</h2>
                <p className="text-gray-500 text-sm mt-1">{clinic.reviews?.length || 0} ta sharh</p>
              </div>
              <button className="text-blue-600 hover:underline text-sm">
                Sharh qoldirish
              </button>
            ...
            <div className="flex items-center gap-6 mb-6 p-4 bg-gray-50 rounded-xl">
              <div className="text-center">
                <div className="text-4xl font-bold text-blue-600">{clinic.rating}</div>
                <div className="flex mt-1">
                  {[...Array(5)].map((_, i) => (
                    <Star 
                      key={i} 
                      className={`w-5 h-5 ${i < Math.floor(clinic.rating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                    />
                  ))}
                </div>
                    ...
                    <div key={star} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${(clinic.reviews?.filter(r => r.rating === star).length / clinic.reviews?.length) * 100 || 0}%` }}
                        />
                      </div>
                      <span className="text-sm w-8 text-right">
                        {clinic.reviews?.filter(r => r.rating === star).length || 0}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-4">
              {clinic.reviews?.map((r, i) => (
                <div key={i} className="border rounded-xl p-4 hover:shadow-sm transition">
                    ...
                    <div>
                      ...
                      <div className="flex mt-1">
                        {[...Array(5)].map((_, starIndex) => (
                          <Star 
                            key={starIndex} 
                            className={`w-4 h-4 ${starIndex < r.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                          />
                        ))}
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow p-6 sticky top-24">
            <h3 className="font-semibold text-lg mb-4 text-gray-800">Aloqa ma'lumotlari</h3>
            ...
            <div className="border-t pt-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="font-semibold text-gray-800">Call Center & Ishonch raqamlar</p>
              </div>
            ...
            <div className="border-t pt-4 mt-4">
              <div className="text-center mb-2">
                ...
                <p className="text-sm text-gray-500">O'rtacha konsultatsiya narxi</p>
              </div>
            ...
            {/* Action Buttons */}
            <button className="mt-6 w-full bg-linear-to-r from-blue-600 to-blue-500 text-white py-3 rounded-xl font-semibold hover:shadow-lg transition">
              Online yozilish
            </button>
          ...
          {/* Map Placeholder */}
          <div className="bg-white rounded-xl shadow p-6">
            <h3 className="font-semibold mb-4 text-gray-800">Manzil xaritada</h3>
            <div className="h-48 bg-gray-200 rounded-lg flex items-center justify-center">
              <MapPin className="w-8 h-8 text-red-500" />
              <span className="ml-2 text-gray-600">Google Xarita integratsiyasi</span>
            </div>
